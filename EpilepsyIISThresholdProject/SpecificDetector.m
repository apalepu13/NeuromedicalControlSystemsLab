...
% being tested.
for i = optimalThreshold:optimalThreshold
...
[evaluation, evalStats] = evalSpikes(approximatedTimes2, spike_times, numSpikes, timeLeniency);
[myFPR, myTPR] = generateROC(evalStats);
num = num + 1;
end



[myoptimalDistance, optimalThresholdIndex] = optimizeROC(myFPR, myTPR); 
...
% end
EEG2 = EEG;
%save('ROC.mat', 'myoptimalDistance', 'myTPR', 'myFPR');
%% FUNCTIONS
...
function [evaluation, evalStats] = evalSpikes(approximatedTimes1, spikeTimes, numSpikes, timeLeniency)
    %Ensures evaluation holds a different value in each scenario.
    evaluation = (2*spikeTimes) + approximatedTimes1; 
    ...
    for i = 1: size(evaluation, 2)
        for j = 1: size(evaluation, 1)
            if evaluation(j, i) == 3
                evalStats(1, 1) = evalStats(1,1) + 1;
            elseif evaluation(j, i) == 2 %could add something to make this only count once...
                evalStats(1, 2) = evalStats(1, 2) + 1;
            elseif evaluation(j, i) == 1
                evalStats(1, 3) = evalStats(1, 3) + 1;
            else
                evalStats(1, 4) = evalStats(1, 4) + 1;
            end
        end
    end
